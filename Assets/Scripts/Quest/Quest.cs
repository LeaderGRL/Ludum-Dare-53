...
public class Quest : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        filteredQuest = filterQuest(quests, player.level);
        page = pagination(filteredQuest);
    ...
    private List<List<JSON.Data>> pagination(List<JSON.Data> filteredQuest)
    {
        List<List<JSON.Data>> res = new();
        List<JSON.Data> page = new List<JSON.Data>();
        for (int i = 0; i < filteredQuest.Count; i++)
        {
            if ((i % 5 == 0 && i != 0) || i == filteredQuest.Count - 1)
            {
